#!/usr/bin/env python3
"""Standalone static server for the claim-map prototype.
...
Usage:
    python3 serve.py --graph /path/to/dist/graph.json [--port 8004]
"""
import argparse
import http.server
import os
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    instead of buffering a multi-megabyte payload per request. All the
    prototypes share this shape on purpose — reading one teaches them all —
...
def main():
    ap = argparse.ArgumentParser(description="Serve the claim-map prototype.")
    ...
    print(f"claim-map serving on http://{args.host}:{args.port}  (graph: {graph_path})")
    try:
        httpd.serve_forever()
